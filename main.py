...
app.logger.info(
    f"Flask app initialized. Production: {IS_PRODUCTION}, Debug: {app.debug}"
)
...
if not app.secret_key and IS_PRODUCTION:
    ...
    app.logger.warning(
        "FLASK_SECRET_KEY not set, using default for development. THIS IS INSECURE FOR PRODUCTION."
    )
...
if IS_PRODUCTION:
    ...
    if os.path.exists(CLIENT_SECRETS_FILE):
        ...
        app.logger.info(
            f"Successfully loaded {CLIENT_SECRETS_FILE} from environment variable."
        )
    else:
        app.logger.critical(
            "FATAL: GOOGLE_CLIENT_SECRET_JSON environment variable not set in production."
        )
...
if IS_PRODUCTION:
    ...
    if not render_url:
        app.logger.warning(
            "RENDER_EXTERNAL_URL not found in production environment! Using Fallback"
        )
...
class EmailService:
    ...
    def send_bulk_email(
        ...
        self.logger.info(
            f"Starting bulk email send to {len(recipients)} recipients. Subject: '{subject}'"
        )
        results = []
        ...
        for recipient in recipients:
            ...
            self.logger.debug(
                f"Processing recipient: {recipient_email} in send_bulk_email loop."
            )
            try:
                ...
                self.logger.info(
                    f"Successfully prepared and sent email for {recipient_email}. Message ID: {result.get('id')}"
                )
            except Exception as e:
                self.logger.error(
                    f"Failed to send email to {recipient_email} (error caught in send_bulk_email): {e}",
                    exc_info=True,
                )
                results.append(
                    {"recipient": recipient_email, "status": "error", "error": str(e)}
                )

        self.logger.info(
            f"Bulk email sending complete. Success: {sum(1 for r in results if r['status'] == 'success')}, Failed: {sum(1 for r in results if r['status'] == 'error')}"
        )
        return results
    ...
    def _send_single_email(
        ...
        try:
            self.logger.debug(
                f"Recipient: {recipient}, Subject: '{subject}', Sender: {sender_email}"
            )
            ...
            self.logger.debug(
                "MIMEMultipart('alternative') created for text/html bodies."
            )
            ...
            self.logger.debug(
                "Multipart/alternative (with plain text) attached to main message."
            )

            self.logger.debug(
                "Attempting to create HTML body (which may embed signature to main 'message')..."
            )
            ...
            if attachments:
                ...
                for file_idx, file_obj in enumerate(attachments):
                    if file_obj and file_obj.filename:
                        self.logger.debug(
                            f"Adding general attachment {file_idx + 1}: {file_obj.filename}"
                        )
                        self._add_attachment(message, file_obj)
                    else:
                        self.logger.debug(
                            f"Skipping general attachment {file_idx + 1} (no file/filename)."
                        )
            else:
            ...
            self.logger.info(
                f"Attempting to send email to {recipient} via Gmail API (after _debug_message)."
            )
            return self._send_via_gmail_api(message)
        except Exception as e:
            self.logger.error(
                f"--- ERROR within _send_single_email for {recipient} BEFORE API call: {e} ---",
                exc_info=True,
            )
            raise

    def _create_html_body(
        self, body: str, message: MIMEMultipart, signature_file
    ) -> str:
        ...
        """
        self.logger.debug(
            f"HTML body created in _create_html_body. Signature included: {bool(signature_html)}"
        )
    ...
    def _get_signature_html(self, msg: MIMEMultipart, signature_file) -> str:
        ...
        if not signature_file or not signature_file.filename:
            self.logger.debug(
                "No signature file provided or filename is empty in _get_signature_html."
            )
            self.logger.debug("--- LEAVING _get_signature_html (no signature) ---")
            return ""

        self.logger.info(
            f"Processing signature file in _get_signature_html: {signature_file.filename}"
        )
        try:
            cid = (
                f"signature_{hashlib.md5(signature_file.filename.encode()).hexdigest()}"
            )
            self.logger.debug(f"Signature CID generated: {cid}")

            self.logger.debug(
                f"Reading signature file content for {signature_file.filename}..."
            )
            file_content = signature_file.read()
            self.logger.debug(
                f"Signature file content read ({len(file_content)} bytes). Seeking to 0 for {signature_file.filename}."
            )
            signature_file.seek(0)
            self.logger.debug(
                f"Creating MIMEImage for signature {signature_file.filename}..."
            )
            ...
            if actual_subtype:
                ...
                self.logger.debug(
                    f"Using subtype '{actual_subtype}' for signature MIMEImage."
                )
            ...
            img.add_header(
                "Content-Disposition", "inline", filename=signature_file.filename
            )
            self.logger.debug(
                f"Attaching MIMEImage (signature) {signature_file.filename} to message..."
            )
            msg.attach(img)
            self.logger.info(
                f"Signature image '{signature_file.filename}' embedded with CID: {cid}"
            )
            ...
            self.logger.error(
                f"Error processing signature file '{signature_file.filename}' in _get_signature_html: {e}",
                exc_info=True,
            )
    ...
    def _add_attachment(self, msg: MIMEMultipart, file_storage_obj):
        self.logger.debug(
            f"--- ENTERING _add_attachment for {file_storage_obj.filename} ---"
        )
        try:
            ...
            self.logger.debug(
                f"Guessed content_type: {content_type}, encoding: {encoding} for {filename}"
            )
            if content_type is None or encoding is not None:
                content_type = "application/octet-stream"
                self.logger.debug(
                    f"Defaulted content_type to 'application/octet-stream' for {filename}"
                )
            ...
            self.logger.debug(
                f"Attachment content read ({len(file_content)} bytes). Seeking to 0 for {filename}."
            )
            ...
            if main_type == "image":
                ...
                self.logger.debug(
                    f"Creating MIMEBase for attachment: {filename}, main_type: {main_type}, sub_type: {sub_type}"
                )
            ...
            self.logger.info(
                f"Successfully created and attached MIME part for attachment: {filename}"
            )
            self.logger.debug(
                f"--- LEAVING _add_attachment for {filename} (SUCCESS) ---"
            )

        except Exception as e:
            self.logger.error(
                f"Error in _add_attachment for {file_storage_obj.filename}: {e}",
                exc_info=True,
            )
            self.logger.debug(
                f"--- LEAVING _add_attachment for {file_storage_obj.filename} (ERROR) ---"
            )
            raise
    ...
    def _debug_message(self, msg: MIMEMultipart):
        ...
        if isinstance(payload, list):
            ...
            for i, part in enumerate(payload):
                self.logger.debug(
                    f"Part {i}: Content-Type='{part.get_content_type()}', "
                ...
                if part.get_content_maintype() == "multipart":
                    self.logger.debug(
                        f"  Sub-Part {i} details ({part.get_content_type()}):"
                    )
                    for j, sub_part in enumerate(part.get_payload()):
                        self.logger.debug(
                            f"    Sub-Part {i}.{j}: Content-Type='{sub_part.get_content_type()}', "
                            f"Charset='{sub_part.get_charset()}', "
                            f"Filename='{sub_part.get_filename()}'"
                        )
        else:
            self.logger.debug(
                f"Message payload (not a list): Content-Type='{msg.get_content_type()}'"
            )
    ...
    def _send_via_gmail_api(self, msg: MIMEMultipart):
        try:
            self.logger.info(
                "Attempting to send email via Gmail API (Method 1: as_string, utf-8 encode)"
            )
            ...
            self.logger.info(
                f"Email sent successfully (Method 1). Message ID: {result.get('id')}"
            )
            return result
        except Exception as e1:
            self.logger.warning(
                f"Gmail API send (Method 1) failed: {e1}", exc_info=True
            )
            self.logger.info(
                "Attempting to send email via Gmail API (Method 2: as_bytes)"
            )
            try:
                ...
                self.logger.info(
                    f"Email sent successfully with (Method 2). Message ID: {result.get('id')}"
                )
                return result
                ...
                self.logger.error(
                    f"Gmail API HttpError (Method 2): {e.resp.status} - {error_content}",
                    exc_info=True,
                )
                raise Exception(f"Gmail API error ({e.resp.status}): {error_content}")
            except Exception as e2:
                self.logger.error(
                    f"Both Gmail API send methods failed. Method 1 error: {e1}, Method 2 error: {e2}",
                    exc_info=True,
                )
                raise Exception(
                    f"Failed to send email after trying multiple methods: {e2}"
                )
...
def get_gmail_service():
    ...
    if not creds or not creds.valid:
        if creds:  # creds exist but are not valid
            logger.warning(
                f"Credentials invalid. Expired: {creds.expired}, Refresh Token: {bool(creds.refresh_token)}"
            )
            if creds.expired and creds.refresh_token:
                ...
                try:
                    ...
                    flask.session.pop(
                        "credentials", None
                    )  # Clear failed refresh attempt
                    flask.session.pop("user_info", None)
                    return None
            else:  # No refresh token or not expired but invalid
                logger.warning(
                    "Credentials invalid and cannot be refreshed. User needs to re-authenticate."
                )
                flask.session.pop("credentials", None)
                flask.session.pop("user_info", None)
                return None
            ...
            logger.info(
                "No valid credentials found in session. User needs to authenticate."
            )
            return None
...
def read_root():
    ...
    if user_info and user_info.get("isAuthorized"):
        ...
        username = user_info.get(
            "name", user_info.get("given_name", user_info.get("email"))
        )
    ...
    return flask.render_template(
        "index.html", isAuthorized=is_authorized, username=username
    )
...
def authorize():
    ...
    try:
        ...
        app.logger.info(
            f"Authorization URL generated. Redirecting user. State: {state}"
        )
        return flask.redirect(authorization_url)
    except FileNotFoundError:
        app.logger.critical(
            f"{CLIENT_SECRETS_FILE} not found. Cannot initiate OAuth flow."
        )
        return (
            f"Error: Server configuration issue ({CLIENT_SECRETS_FILE} missing). Please contact admin.",
            500,
        )
    except Exception as e:
...
def login_callback():
    ...
    try:
        ...
        if not all_scopes_granted:
            app.logger.error(
                f"Required permissions not granted by user. Missing: {missing_scopes}"
            )
            flask.session.clear()  # Clear all session data for clean re-auth
            return (
                f"Error: Required permissions not granted ({', '.join(missing_scopes)}). Please ensure all requested scopes are approved during login.",
                403,
            )
        ...
        app.logger.info(
            f"User info fetched and stored in session for: {user_info.get('email')}"
        )
        ...
        app.logger.critical(
            f"{CLIENT_SECRETS_FILE} not found during callback. Cannot complete OAuth flow."
        )
        flask.session.clear()
        return (
            f"Error: Server configuration issue ({CLIENT_SECRETS_FILE} missing). Please contact admin.",
...
def compose():
    ...
    if request.method == "GET":
        ...
        if not user or not user.get("isAuthorized"):  # Check authorization flag
            logger.warning(
                "User not authenticated or not authorized for compose page. Redirecting to login."
            )
    ...
    if request.method == "POST":
        logger.info(
            "Compose route '/compose' accessed (POST) - attempting to send email."
        )
        try:
            if "user_info" not in flask.session or not flask.session["user_info"].get(
                "isAuthorized"
            ):
                logger.warning(
            ...
            logger.info(
                f"Form data received: Recipients: {len(recipients)}, Subject: '{subject}', CC: '{cc}', BCC: '{bcc}'"
            )
            ...
            if files:
                ...
                for file_idx, file_storage in enumerate(
                    files
                ):  # file_storage is a FileStorage object
                    if file_storage and file_storage.filename:
                        ...
                        if not secured_filename:  # secure_filename might return empty if original is dangerous/empty
                            logger.warning(
                                f"Filename '{original_filename}' became empty after securing. Skipping."
                            )
                            continue
                        ...
                        logger.debug(
                            f"Attachment {file_idx + 1}: '{original_filename}' (secured to '{file_storage.filename}') added to list."
                        )
                    elif file_storage and not file_storage.filename:
                        logger.debug(
                            f"Attachment {file_idx + 1} has no filename, skipping."
                        )
            else:
                logger.info("No file attachments found in request.")

            signature_file_from_form = None
            signature_file_storage = request.files.get(
                "signature"
            )  # FileStorage object
            if signature_file_storage and signature_file_storage.filename:
                ...
                if not secured_sig_filename:
                    logger.warning(
                        f"Signature filename '{original_sig_filename}' became empty after securing. Not using signature."
                    )
                else:
                    signature_file_storage.filename = secured_sig_filename  # Overwrite
                    signature_file_from_form = signature_file_storage
                    logger.info(
                        f"Signature file '{original_sig_filename}' (secured to '{signature_file_storage.filename}') found."
                    )
            else:
...
if __name__ == "__main__":
    if not IS_PRODUCTION:
        ...
        app.logger.info(
            "Running in production mode - Gunicorn should be managing this process."
        )
